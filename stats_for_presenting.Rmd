...
applicantData <- na.omit(applicantData)
 
categorical_vars <- c("program", "degree", "entry_term", "time_status", 
                      "sex", "birth_country", "native_language", 
                      "citizenship_status", "citizenship", "failed_course", 
                       "inst_1_name", "inst_1_location", 
                      "inst_1_study_level", "inst_1_degree", "inst_1_major", 
                      "currently_employed", "heard_about_UR", 
                      "other_schools_applying")
 
#print(categorical_vars)
 
categorical_vars <- categorical_vars[categorical_vars %in% colnames(applicantData)]
print(categorical_vars)
 
applicantData[categorical_vars] <- lapply(applicantData[categorical_vars], factor)
 
applicantData$decision <- as.factor(ifelse(applicantData$decision == "Admit/Accept offer", 1, 0))
 
 
#Splitting the dataset into 70% Training and 30% Testing
set.seed(123) # for reproducible results
split <- sample.split(applicantData$decision, SplitRatio = 0.7)
train <- subset(applicantData, split == TRUE)
test <- subset(applicantData, split == FALSE)

```

```{r logistic regression - model building, echo=FALSE}

model <- glm(decision ~ ., data = applicantData, family = "binomial")
print(summary(model))
...
conf_matrix <- table(Predicted = predicted_classes, Actual = test$decision)
print(conf_matrix)
