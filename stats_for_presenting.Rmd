...
```{r, echo=FALSE}
write.csv(masters_data,"masters.csv")
write.csv(phd_data,"pdh.csv")
```
...
m_proportions_accepted <- m_proportions_df %>% filter(Var3 == "accept")
m_proportions_M <- m_proportions_accepted[m_proportions_accepted$Var1 == "M", ]
m_proportions_F <- m_proportions_accepted[m_proportions_accepted$Var1 == "F", ]
m_merged_proportions <- rbind(m_proportions_M, m_proportions_F)
print(m_merged_proportions)


ggplot(m_merged_proportions, aes(x = Var2, y = Freq , fill=Var1)) +
  geom_col(position = "fill")+ labs(title="U of R Student Application - Master's Acceptance Rate Based On Gender Across Different Terms (2015 -2022)", x = "Term Year", y = "Proportion", fill="Gender") + theme(plot.title = element_text(size=9))
...
p_proportions_accepted <- p_proportions_df %>% filter(Var3 == "accept")
p_proportions_M <- p_proportions_accepted[p_proportions_accepted$Var1 == "M", ]
p_proportions_F <- p_proportions_accepted[p_proportions_accepted$Var1 == "F", ]
p_merged_proportions <- rbind(p_proportions_M, p_proportions_F)
print(p_merged_proportions)


ggplot(p_merged_proportions, aes(x = Var2, y = Freq , fill=Var1)) +
  geom_col(position = "fill")+ labs(title="U of R Student Application - PhD Acceptance Rate Based On Gender Across Different Terms (2015 -2022)", x = "Term Year", y = "Proportion", fill="Gender") + theme(plot.title = element_text(size=9))
```
...
m_citizen_proportions_accepted <- m_citizen_proportions_df %>% filter(Var3 == "accept")
m_proportions_FN <- m_citizen_proportions_accepted[m_citizen_proportions_accepted$Var1 == "FN", ]
m_proportions_US <- m_citizen_proportions_accepted[m_citizen_proportions_accepted$Var1 == "US", ]
m_proportions_PR <- m_citizen_proportions_accepted[m_citizen_proportions_accepted$Var1 == "PR", ]
m_proportions_UN <- m_citizen_proportions_accepted[m_citizen_proportions_accepted$Var1 == "unknown", ]
m_merged_citizen <- rbind(m_proportions_FN, m_proportions_US,m_proportions_PR,m_proportions_UN)
...
p_citizen_proportions_accepted <- p_citizen_proportions_df %>% filter(Var3 == "accept")
p_proportions_FN <- p_citizen_proportions_accepted[p_citizen_proportions_accepted$Var1 == "FN", ]
p_proportions_US <- p_citizen_proportions_accepted[p_citizen_proportions_accepted$Var1 == "US", ]
p_proportions_PR <- p_citizen_proportions_accepted[p_citizen_proportions_accepted$Var1 == "PR", ]
p_proportions_UN <- p_citizen_proportions_accepted[p_citizen_proportions_accepted$Var1 == "unknown", ]
p_merged_citizen <- rbind(p_proportions_FN, p_proportions_US,p_proportions_PR,p_proportions_UN)
...
pie_chart <- ggplot(m_pro, aes(x = "", y = applicant, fill = citizenship_status)) +
...
ggplot(data, aes(x = !!sym(plot_column))) +    geom_histogram(binwidth = binwidth, fill = fill_color, color = "black") +    theme_minimal() +    labs(title = text,         x = plot_column,         y = "Frequency") +    theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
} 

create_filtered_histogram(masters_data, "age", "failed_course", 0, 0.5, "green","U of R Student Application - Master's Applicants Never Failed a Course (2015 -2022)") 

create_filtered_histogram(masters_data, "age", "failed_course", 1, 0.5, "yellow","U of R Student Application - Master's Applicants Ever Failed a Course (2015 -2022)") 

ggplot(applicantData, aes(x = degree, y = age)) +  geom_boxplot() +  theme_minimal() +  labs(title = "U of R Student Application - Age Distribution by Degree (2015 -2022)",       x = "Degree",       y = "Age") +  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
